  ...
  <v-card>
    <v-container v-if="quiz">
      ...
      <v-stepper v-model="step" v-else>
        ...
        <v-stepper-items>
          <v-stepper-content v-for="(question, i) in quiz.questions" :key="i" :step="i + 1">
            <h3>{{question.question}}</h3>
            <v-radio-group v-model="stepAnswer">
              <v-radio v-for="(answer, j) in question.answers" :key="j" :label="answer.answer" :value="j"></v-radio>
            </v-radio-group>
