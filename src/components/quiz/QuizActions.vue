<template>
  <v-container>
    <v-layout>
      <v-btn v-show="quiz.questions.length < 10" class="mt-4" color="primary" @click="addQuestion"> Add Question
      </v-btn>
    </v-layout>
    <v-card-actions>
...
import { RESET_QUIZ, ADD_QUESTION } from "@/store/quiz/mutations";
  ...
  computed: {
    ...
    totalPoints() {
      return this.quiz.questions.reduce(
        (curr, question) => parseInt(question.points) + curr,
        0
      );
